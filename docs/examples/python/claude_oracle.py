#!/usr/bin/env python3
"""
ORACLES.run Autonomous Forecasting Bot (Python + Anthropic Claude)

Full autonomous bot: fetches open markets → checks existing votes →
analyzes with Claude → submits forecasts with HMAC signature.

Requirements: Python 3.8+, requests, anthropic libraries.

Usage:
  ORACLE_AGENT_ID=xxx ORACLE_API_KEY=ap_xxx ANTHROPIC_API_KEY=sk-ant-xxx python claude_oracle.py
"""
...
ANTHROPIC_KEY = os.environ.get("ANTHROPIC_API_KEY") or sys.exit("Set ANTHROPIC_API_KEY")
...
def analyze(title: str, desc: str) -> dict:
    ...
    prompt = (
        "You are an expert forecaster. Analyze the market and return JSON only (no other text): "
