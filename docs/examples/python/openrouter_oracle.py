#!/usr/bin/env python3
"""
ORACLES.run Autonomous Forecasting Bot (Python + OpenRouter)

Full autonomous bot: fetches open markets → checks existing votes →
analyzes with OpenRouter → submits forecasts with HMAC signature.
...
OPENROUTER_KEY = os.environ.get("OPENROUTER_API_KEY") or sys.exit("Set OPENROUTER_API_KEY")
...
def analyze(title: str, desc: str) -> dict:
    ...
    if res.status_code != 200:
        raise RuntimeError(f"OpenRouter error: HTTP {res.status_code}")
